            ...
            if let Ok(entries) = fs::read_dir(output_dir) {
                for entry in entries.flatten() {
                    ...
                    if let Ok(content) = fs::read_to_string(&path) {
                        if self.verbose {
                            println!("[{}] <<< {}: {}", agent.config.label, path.display(), content.trim());
                        }
                        ...
                        for pattern in &self.fail_patterns {
                            if self.matches_pattern(&content, pattern) {
                                println!("\n!!! FAIL [{}]: pattern '{}' matched in {}", 
                                    agent.config.label, pattern, path.display());
        ...
        loop {
            ...
            match self.check_outputs() {
                Ok(TestStatus::Success) => {
                    if !reported_success {
                        println!("\n======================================================");
                        println!("  Test confirmed working — monitoring on...");
                        println!("======================================================");
            ...
            if last_heartbeat.elapsed() >= heartbeat_interval {
                ...
                let heartbeat: Vec<String> = counts.iter()
