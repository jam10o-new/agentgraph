...
impl CompressionManager {
    pub fn new(base_path: PathBuf, threshold: f64, inverse_prob: f64, resummarize_prob: f64) -> Self {
        Self {
    ...
    fn specialized_summary_path(&self, turn_idx: usize, domain: &str) -> PathBuf {
        self.turn_cache_dir(turn_idx).join(format!("domain_{}.json", domain))
    }
    ...
    pub async fn get_compressed_context(
        ...
        for (i, turn) in history.iter().enumerate() {
            ...
            let prob = self.inverse_prob.powf(depth as f64);
            ...
            if prob < self.threshold {
                let compressed = self.process_turn_compression(
                    model.clone(),
                    history,
                    i,
                    latest_turn,
    ...
    async fn process_turn_compression(
        ...
        let content_hash = xxhash_rust::xxh3::xxh3_64(turn.content.as_bytes());
        ...
        let mut root = if root_path.exists() {
            ...
            if r.content_hash != content_hash || should_resummarize {
                self.generate_root_summary(model.clone(), history, turn_idx, sampling.clone(), Some(r)).await?
            } else {
                r
            }
        } else {
            self.generate_root_summary(model.clone(), history, turn_idx, sampling.clone(), None).await?
        };
        ...
        if root.domains.is_empty() {
            if self.is_summary_sufficient(model.clone(), &root, latest_turn, sampling.clone()).await? {
                return Ok((role_to_mistral(&turn.role), root.micro_summary.clone()));
            } else {
                let spec = self.generate_specialized_summary(model.clone(), turn, latest_turn, sampling.clone()).await?;
                root.domains.push(spec.domain.clone());
                fs::write(&root_path, serde_json::to_string_pretty(&root)?).await?;
                fs::write(self.specialized_summary_path(turn.turn_index, &spec.domain), serde_json::to_string_pretty(&spec)?).await?;
                return Ok((role_to_mistral(&turn.role), spec.extracted_information));
            }
        }

        // 4) selection logic for > 1 summary
        let selected_domains = self.select_relevant_domains(model.clone(), &root, latest_turn, sampling.clone()).await?;
        if selected_domains.is_empty() || (selected_domains.len() == 1 && selected_domains[0] == "root") {
    ...
    async fn generate_root_summary(
        ...
        if let Some(er) = &existing_root {
            system_prompt.push_str(&format!("\nRefine the existing summary considering these other domain summaries: {:?}", er.domains));
        }

        let prompt = format!("Summarize this turn within its local context.\n\nLocal Context:\n{}\n\nTarget Turn Content: {}", context_text, turn.content);
        ...
        let content = resp.choices[0].message.content.as_ref().context("Empty summary")?;
        ...
        let root: RootSummary = if let Some(start_json) = content.find('{') {
            if let Some(end_json) = content.rfind('}') {
                ...
                let mut r: RootSummary = serde_json::from_str(json_str).map_err(|e| anyhow!("JSON Parse Error: {}, content: {}", e, json_str))?;
                r.content_hash = xxhash_rust::xxh3::xxh3_64(turn.content.as_bytes());
                r.included_turn_indices = window_indices;
                if let Some(er) = existing_root {
                    r.domains = er.domains;
                }
                r
            } else { anyhow::bail!("No JSON found in model response") }
        } else { anyhow::bail!("No JSON found in model response") };

        Ok(root)
    }

    async fn is_summary_sufficient(&self, model: Arc<Model>, root: &RootSummary, latest_turn: &str, sampling: SamplingParams) -> Result<bool> {
        let prompt = format!(
        ...
        let req = RequestBuilder::from(messages).set_sampling(sampling);
        let resp: ChatCompletionResponse = model.send_chat_request(req).await?;
        let content = resp.choices[0].message.content.as_ref().map(|s| s.trim().to_uppercase()).unwrap_or_default();
        Ok(content.contains("YES"))
    }

    async fn generate_specialized_summary(&self, model: Arc<Model>, turn: &HistoryTurn, latest_turn: &str, sampling: SamplingParams) -> Result<SpecializedSummary> {
        let prompt = format!(
        ...
        let req = RequestBuilder::from(messages).set_sampling(sampling);
        let resp: ChatCompletionResponse = model.send_chat_request(req).await?;
        let content = resp.choices[0].message.content.as_ref().context("Empty specialized summary")?;
    ...
    async fn select_relevant_domains(&self, model: Arc<Model>, root: &RootSummary, latest_turn: &str, sampling: SamplingParams) -> Result<Vec<String>> {
        ...
        let req = RequestBuilder::from(messages).set_sampling(sampling);
        let resp: ChatCompletionResponse = model.send_chat_request(req).await?;
        let content = resp.choices[0].message.content.as_ref().unwrap_or(&"".to_string()).clone();
        
        let selected: Vec<String> = content.split(',')
...
pub fn turn_to_mistral(turn: &HistoryTurn) -> (TextMessageRole, String) {
    ...
    let content = match &turn.role {
        HistoryTurnRole::Skill(name, desc) => format!("## Skill: {}\n{}\n\n{}", name, desc, turn.content),
